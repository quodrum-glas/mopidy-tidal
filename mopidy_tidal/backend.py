...
class TidalBackend(ThreadingActor, backend.Backend):
    ...
    def _ext(self, key: str) -> object:
        return self._config[self.EXT].get(key)

    def on_start(self) -> None:
        client_id: str = self._ext("client_id") or ""
        client_secret: str = self._ext("client_secret") or ""
        quality: str = self._ext("quality")
        data_dir: Path = Extension.get_data_dir(self._config)
        token_file: Path = data_dir / f"tidal.oauth.{client_id}.json"
